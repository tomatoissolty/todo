...
// --- Writing(글쓰기 앱) 매핑 — 본문은 속성이 아니라 페이지 내용(블록)으로 들어감, 사진 없음 ---
...
const APPS = {
  ...
  diary: {
    ...
    useCover: false,
    hasBodyContent: true // 본문/사진이 속성이 아니라 페이지 블록으로 들어가는 앱
  },
  writing: {
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // --- 본문이 페이지 블록으로 들어가는 앱(일기, 글쓰기) 공용 처리: 항목 하나 저장(속성 + 본문 블록) ---
    if (appConfig.hasBodyContent && req.method === 'POST' && !req.query.action) {
      ...
      if (pageId) {
        ...
        const data = await r.json();
        if (!r.ok) throw new Error(data.message || '업데이트 실패');
      } else {
        ...
        const data = await r.json();
        if (!r.ok) throw new Error(data.message || '생성 실패');
        pageId = data.id;
      }
    ...
    // --- 본문이 페이지 블록으로 들어가는 앱(일기, 글쓰기) 공용 처리: 본문 내용 가져오기 (pull/편집 시 사용) ---
    if (appConfig.hasBodyContent && req.method === 'GET' && req.query.action === 'content') {
